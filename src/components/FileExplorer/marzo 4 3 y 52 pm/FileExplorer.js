  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      for (let i = 0; i < uploadedFiles.length; i++) {
        ...
        // Cambio: usar storageService para subir archivos
...
const renameItem = async (item) => {
  ...
  try {
    ...
    if (isFolder) {
      // Para carpetas, necesitamos manejar de manera especial el renombrado
      // 1. Crear la nueva carpeta
      console.log("Creando nueva carpeta:", newPath);
      ...
      await storageService.uploadFile(
        new Blob([JSON.stringify(folderData)], { type: 'application/json' }), 
        `${newPath}/.folder`
      );
      
      // 2. Eliminar la carpeta antigua
      console.log("Eliminando carpeta antigua:", item.path);
      await storageService.deleteFile(item.path);
    } else {
...
<div 
  ...
  style={{
    ...
    borderRadius: '8px',
    margin: '5px',
    padding: '10px 10px 30px 10px', // Ajustado el padding inferior
    height: '120px', // Establecer una altura fija
    position: 'relative',
    display: 'flex',
    flexDirection: 'column',
    alignItems: 'center',
    justifyContent: 'flex-start', // Alinear contenido al principio
    background: 'white',
...
<React.Fragment key={file.path}>
  ...
  <tr className="file-row file-name-row">
